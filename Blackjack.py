# Made by Jordan Leich on 6/6/2020, Last updated on 7/23/2020
...
def another_game():
    ...
    print(green + "Your win count is", user_score, 'and your total balance is $' + str(user_balance), '\n', reset)
    time.sleep(1)
    ...
    if user_balance <= 0:
        ...
        if dealer_balance <= 0:
            print(green + 'Congratulations! You have beat BlackJack 21 by defeating the dealers balance!\n', reset)
            time.sleep(2)

        elif dealer_balance <= 2500:
            print(green + 'The dealers balance is looking small enough for you to win! Your doing well...\n', reset)
            time.sleep(2)
        ...
        if restart_action.lower() == 'yes' or restart_action.lower() == 'y':
            ...
            print(green + 'You earned a total of $' + str(user_balance) + str('. Thanks for playing!\n'), reset)
            time.sleep(1)
            quit()
...
def game():
    ...
    while len(player_cards) != 2:
        ...
        if len(player_cards) == 2:
            print(green + "You have a total of", str(sum(player_cards)), 'from these cards', player_cards, reset, '\n')
            time.sleep(1)
    ...
    while sum(player_cards) < 21:
        action_taken = str(input("Do you want to hit, stay, call help, or quit the game (hit | stay | help | quit): "))
        print()
        time.sleep(1)

        if action_taken.lower() == "hit" or action_taken.lower() == "h":
            player_cards.append(random.randint(1, 11))
            print(green + "You now have a total of " + str(sum(player_cards)) + " from these cards", player_cards,
                  reset, '\n')
            time.sleep(1)

        elif action_taken.lower() == "help" or action_taken.lower() == "Help":
            if sum(player_cards) <= 14:
                print(blue + "Since your total is under or equal to a total of 14, we recommend that you hit!\n", reset)
                time.sleep(1)
                ...
                time.sleep(3)
                print(blue + "If your card total is closer to 21, don't risk it! make a stay move!\n", reset)
                time.sleep(3)
            ...
            print(red + "The Dealer has a new total of", str(sum(dealer_cards)), 'from these cards', dealer_cards,
                  reset, '\n')
            time.sleep(1)
            print(green + "You have a grand total of " + str(sum(player_cards)) + " with", player_cards, reset, '\n')
            time.sleep(1)

            if sum(dealer_cards) > 21:
                print(green + "The Dealer BUSTED! You win! You won $" + str(user_bet) + '!\n', reset)
                time.sleep(1)
                user_score += 1
                user_balance = user_balance + user_bet
                dealer_balance -= user_bet
                ...
                print(red + "The dealer wins! You lost $" + str(user_bet) + '!\n', reset)
                time.sleep(1)
                user_score -= 1
                user_balance = user_balance - user_bet
                dealer_balance += user_bet
                ...
                print(red + "BLACKJACK! The Dealer hit 21! You lost $" + str(user_bet) + '!\n', reset)
                time.sleep(1)
                user_score -= 1
                user_balance = user_balance - user_bet
                dealer_balance += user_bet
    ...
    if sum(player_cards) > 21:
        print(red + "BUSTED! The Dealer Wins! You lost $" + str(user_bet) + '!\n', reset)
        time.sleep(1)
        user_score -= 1
        user_balance = user_balance - user_bet
        dealer_balance += user_bet
