...
def view_stats(): # revamp to work and give actual stats
...
class Blackjack:
    ...
    def player_balance(self):
        print(colors.green + f"Your balance is ${str(self.user_balance)}\n", colors.reset)
        time.sleep(1)
        print(colors.red + f"The dealers balance is ${str(self.dealer_balance)}\n", colors.reset)
        time.sleep(1)

    def donation_from_bot():
        for c, balance in enumerate(self.bot_balances):
            if balance > 0:
                ...
                print(colors.green + f'Player{c} has donated ${str(bot_donation_amount)} to you.\n', colors.reset)
                time.sleep(1)
                break

    def play_game(self):
        while self.user_balance > 0 or max(self.bot_balances) > 0: # play until all are broke
            self.player_balance()

            # donate to bots only if they don't have any money
            for c, value in enumerate(self.bot_balances):
                if value <= 0:
                    self.bot_balances[c], self.user_balance = donate_to_bot(value, user_balance, f'Player{c}')
            ...
            print(colors.red + f"The Dealer has ? & {str(self.dealer_cards[1])}\n", colors.reset)
            time.sleep(1)
            # player gets cards
            self.user_cards = [random.randint(1, 11) for i in range(2)]
            print(colors.green + f"You have a total of {str(sum(self.user_cards))} from {self.user_cards}\n", colors.reset)
            time.sleep(1)
    ...
    def bot_decisions(self, value): 
        ...
        while sum(self.bot_cards[value]) <= 15:
            if len(self.bot_cards[value]) == 5:
                ...
                time.sleep(1)
                print(colors.blue + f"Player{value + 1} now has a total of " + str(sum(self.bot_cards[value])) + " from these cards",
    ...
    def dealer_draws_card(self):
        ...
        time.sleep(1)
        print(colors.red + f"The Dealer now has {str(sum(self.dealer_cards))} from these cards {self.dealer_cards}\n", colors.reset)
        time.sleep(1)
    ...
    def game_scoring(self):
        ...
        if len(self.dealer_cards) == 5 and sum(self.dealer_cards) <= sum(self.user_cards) <= 21:
            ...
            player_lose_text("BLACKJACK! Since you did not purchased insurance, you will lose 1.5 times your original bet!")
            ...
            print(colors.green + "You Win! Your cards were greater than the dealers deck, You won $" + str(self.user_bet) +
                  "!\n", colors.reset)
            self.user_win_stats()
        elif sum(self.dealer_cards) > sum(self.user_cards):
            print(colors.red + "The dealer wins! Your cards were less than the dealers deck, You lost $" + str(self.user_bet) +
                  "!\n", colors.reset)
            self.user_loses_stats()
    ...
    Used when 1 single round of blackjack has ended. Allows the user to play another game of blackjack or quit playing
        """
        print(colors.green + "Your win count is", self.user_score, "and your total balance is $" + str(self.user_balance), "\n",
              colors.reset)
        time.sleep(1)
...
def bot_player_choice():
    ...
    while True:
        ...
        if player_num == 1:
            ...
            return player_num
        elif player_num > 1 and player_num < 4:
...
Allows the end-user to be able to play the game with custom money, win counts, and more
    ...
    while True:
        ...
        if game_choice.lower() in ['b', 'blackjack', 'blackjack 21', 'black jack', 'n', 'normal']:
            game()
...
### TO DO ###
# add load save game -- not being used currently
# user input validation 
# fix view_stats + reset_stats
